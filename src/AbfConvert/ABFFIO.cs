...
namespace AbfConvert
{
    public static class ABFFIO
    {
        ...
        [DllImport("ABFFIO.dll", CharSet = CharSet.Ansi)]
        public static extern bool ABF_ReadOpen(String szFileName, ref Int32 phFile, UInt32 fFlags, ref ABFFileHeader pFH, ref UInt32 puMaxSamples, ref UInt32 pdwMaxEpi, ref Int32 pnError);

        [DllImport("ABFFIO.dll", CharSet = CharSet.Ansi)]
        public static extern bool ABF_Close(Int32 nFile, ref Int32 pnError);

        [DllImport("ABFFIO.dll", CharSet = CharSet.Ansi)]
        public static extern bool ABF_ReadTags(Int32 nFile, ref ABFFileHeader pFH, UInt32 dwFirstTag, ref ABFTag pTagArray, UInt32 uNumTags, ref Int32 pnError);

        [DllImport("ABFFIO.dll", CharSet = CharSet.Ansi)]
        public static extern bool ABF_ReadChannel(Int32 nFile, ref ABFFileHeader pFH, Int32 nChannel, Int32 dwEpisode, ref float pfBuffer, ref UInt32 puNumSamples, ref Int32 pnError);

        [DllImport("ABFFIO.dll", CharSet = CharSet.Ansi)]
        public static extern int ABFH_GetEpochDuration(ref ABFFileHeader pFH, Int32 nChannel, Int32 dwEpisode, Int32 nEpoch);

        [DllImport("ABFFIO.dll", CharSet = CharSet.Ansi)]
        public static extern float ABFH_GetEpochLevel(ref ABFFileHeader pFH, Int32 nChannel, Int32 dwEpisode, Int32 nEpoch);
        ...
        private const int ABF_DACCOUNT = 8; // number of DAC channels supported.
        private const int ABF_EPOCHCOUNT = 50; // number of waveform epochs supported.
        private const int ABF_ADCUNITLEN = 8; // length of ADC units strings
        private const int ABF_ADCNAMELEN = 10; // length of actual ADC channel name strings
        private const int ABF_DACUNITLEN = 8; // length of DAC units strings
        private const int ABF_DACNAMELEN = 10; // length of DAC channel name strings
        private const int ABF_USERLISTLEN = 256; // length of the user list (V1.6)
        private const int ABF_USERLISTCOUNT = ABF_DACCOUNT; // number of independent user lists (V1.6)       
        private const int ABF_FILECOMMENTLEN = 128; // length of file comment string (V1.6)
        private const int ABF_PATHLEN = 256; // length of full path, used for DACFile and Protocol name.
        private const int ABF_CREATORINFOLEN = 16; // length of file creator info string
        private const int ABF_ARITHMETICOPLEN = 2; // length of the Arithmetic operator field
        private const int ABF_ARITHMETICUNITSLEN = 8; // length of arithmetic units string
        private const int ABF_STATS_REGIONS = 24; // The number of independent statistics regions. // ST-91
        private const int ABF_TAGCOMMENTLEN = 56; // length of tag comment string
        private const int ABFH_HOLDINGFRACTION = 64; // helps calculate the pre-epoch duration

        // These constants are ready to go, but not currently needed for anything
        //const int ABF_ADCNAMELEN_USER = 8; // length of user-entered ADC channel name strings
        //const int ABF_OLDFILECOMMENTLEN = 56; // length of file comment string (pre V1.6)
        //const int ABF_BLOCKSIZE = 512; // Size of block alignment in ABF files.
        //const int PCLAMP6_MAXSWEEPLENGTH = 16384; // Maximum multiplexed sweep length supported by pCLAMP6 apps.
        //const int PCLAMP7_MAXSWEEPLEN_PERCHAN = 1032258; // Maximum per channel sweep length supported by pCLAMP7 apps.
        //const int PCLAMP11_MAXSWEEPLEN_PERCHAN = 5161290; // Maximum per channel sweep length supported by pCLAMP11 apps. //ST-1
        //const int ABF_MAX_SWEEPS_PER_AVERAGE = 65500; // The maximum number of sweeps that can be combined into a cumulative average 
        //const int ABF_MAX_TRIAL_SAMPLES = 0x7FFFFFFF; // Maximum length of acquisition supported (samples)
    ...
    // TODO: make this an enum or dictionary
    /*
